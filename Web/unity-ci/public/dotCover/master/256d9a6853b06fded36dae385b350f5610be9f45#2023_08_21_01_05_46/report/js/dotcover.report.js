...
function getNode(index) {
  ...
  node.line = data[4];
  node.column = data[5];
  node.metadataIndex = data[6];
  node.firstChildIndex = data[7];
  node.childrenNumber = data[8];
  return node;
}
...
function navigateToNode(node) {
  var data = node.data;
  var url = resourceFolder + "/src/" + (data.fileNameIndex ? (data.fileNameIndex + "_" + data.metadataIndex + ".html#s" + data.line + "." + data.column) : "nosource.html");
